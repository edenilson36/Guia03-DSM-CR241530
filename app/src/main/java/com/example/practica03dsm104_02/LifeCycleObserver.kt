...
/**
 *  Se llama la actividad entra en estado Started (visible para el usuario)
 */
...
override fun onStart(owner: LifecycleOwner) {
    ...
    activity.window.decorView.setBackgroundColor(Color.GREEN) // Cambia fondo verde
...
override fun onPause(owner: LifecycleOwner) {
    contadorSuspendido++ // Incrementa contador de veces suspendido txtEstado.text = "Estado actual: onPause" // Actualiza texto de estado
    activity.window.decorView.setBackgroundColor(Color.YELLOW) // Cambia fondo a amarillo
    actualizarPantalla() // Actualiza los TextViews con los contadores
}

/**
 * Se llama cunado la actividad es destruidad (cerrada)
 */

override fun onDestroy(owner: LifecycleOwner){
    contadorCerrado++ // Incrementa contador de veces cerrado
    txtEstado.text = "Estado actual: onDestroy" // Actializa texto de estado
